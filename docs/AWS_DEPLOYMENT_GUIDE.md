...
- [ ] **로컬 컴퓨터에 설치할 도구들**
  ...
  - [ ] jq (JSON 처리 도구)
- [ ] **도메인** 1개 (예: devworld.cloud) - Cloudflare 또는 Route53에서 관리
- [ ] **외부 서비스 계정**
...
├── ALB → airflow.devworld.cloud (HTTPS)
├── ECR (Docker 이미지 저장소)
...
> **HTTPS 설정 전에는** `http://`로 접속합니다. 9단계 이후에는 `https://airflow.devworld.cloud`으로 접속합니다.
...
```bash
# HTTP -> HTTPS 리다이렉트 확인
curl -I http://airflow.devworld.cloud
```

**예상 출력:**
```
HTTP/1.1 301 Moved Permanently
Location: https://airflow.devworld.cloud:443/
```

```bash
# HTTPS 접속 확인
curl -I https://airflow.devworld.cloud
```
...
브라우저에서 `https://airflow.devworld.cloud`으로 접속하여 Airflow UI가 정상적으로 표시되는지 확인합니다.
