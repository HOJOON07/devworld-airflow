...
| dlt | 데이터 적재 (RSS/API/structured discovery → load) |
| dbt | 데이터 변환 (bronze → silver → gold, mart 생성) |

**Airflow 실행 방식**
| 항목 | 값 |
|---|---|
| Executor | LocalExecutor |
| 구성 요소 | webserver + scheduler (ECS Fargate 각 1서비스) |
...
│       ├── airflow-pipeline.md            #     Airflow Pipeline Engineer (DAG/ETL)
...
### Layer 3: Specialization (Agents) — 확정 7명
...
| `airflow-runtime` | Airflow Runtime Engineer | Airflow 설정, executor, connection, 스케줄 | Read, Edit, Write, Bash | opus |
| `airflow-pipeline` | Airflow Pipeline Engineer | DAG, dlt, dbt, 크롤러/파서, dedup | Read, Edit, Write, Bash | opus |
| `infra-engineer` | Terraform / Cloud Infra Engineer | Terraform, Docker, ECS, VPC, CloudWatch | Read, Edit, Write, Bash | opus |

> 전원 claude-opus-4-6. 읽기 전용(lead/reviewer/architect) vs 쓰기 가능(runtime/pipeline/infra) 분리.
...
- [x] agents/ — 7개 서브에이전트 정의 (전원 opus 4.6)
- [x] 읽기 전용(lead/reviewer/architect) vs 쓰기 가능(runtime/pipeline/infra) 분리

### Phase 4: Workflow (Level 4)
- [x] 팀 스킬 3개 (team-parallel-dev, team-review, team-research)
- [x] 단독 스킬 3개 (dag-scaffold, dag-review, debug-dag)

### Phase 5: Orchestration (Level 5)
- [x] Agent Teams 활성화 (settings.json env)
- [ ] cmux 연동 설정
- [ ] 실제 DAG 개발 작업에서 워크플로우 검증
- [ ] 피드백 → 하네스 최적화 루프 시작
